...
export const getTop3Rank = () => function (dispatch){
     intercept.get('/api/rank/top3'
        ...
        const newTop3 = [{
            ...
            totalBalance: Number(reusltData[1]?.totalBalance === undefined? 0 : reusltData[1]?.totalBalance),
            fluctuationRate: reusltData[1]?.fluctuationRate === undefined? 0 : reusltData[1]?.fluctuationRate,
          },
          {
            ...
            totalBalance: Number(reusltData[0]?.totalBalance === undefined? 0 : reusltData[0]?.totalBalance),
            fluctuationRate: reusltData[0]?.fluctuationRate === undefined? 0 : reusltData[0]?.fluctuationRate,
          },
          {
            ...
            totalBalance: Number(reusltData[2]?.totalBalance === undefined? 0 : reusltData[2]?.totalBalance),
...
// 유저인포
export const getUserInfo = () => function (dispatch){
  axios.get('/api/rank/userinfo'
  ).then((response) => {
...
// 유저 1:1 매치업
export const getMatchUp = () => function (dispatch){
  axios.get('/api/rank/matchup'
  ).then((response) => {
